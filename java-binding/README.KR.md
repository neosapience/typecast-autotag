...
전화번호, 날짜, 금액 등 다양한 패턴을 한국어 음성 합성에 적합한 형식으로 자동 변환합니다.
...
VERSION=$(curl -s https://api.github.com/repos/typecast-ai/typecast-autotag-ts/releases/latest | grep '"tag_name"' | sed -E 's/.*"v([^"]+)".*/\1/')
curl -L -o typecast-autotag-${VERSION}.jar \
  https://github.com/typecast-ai/typecast-autotag-ts/releases/download/v${VERSION}/typecast-autotag-${VERSION}.jar
...
```bash
# 저장소 클론
git clone https://github.com/typecast-ai/typecast-autotag-ts.git
...
# 또는 웹 인터페이스 사용:
# https://github.com/typecast-ai/typecast-autotag-ts/releases/new
```
...
- [typecast-autotag-ts](../README.md) - TypeScript/JavaScript 버전
