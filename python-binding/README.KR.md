...
Python 3.10 이상이 필요합니다. Python 3.8과 3.9는 EOL로 인해 지원이 중단되었습니다. 발행된 메타데이터 기준 해당 Python 버전을 마지막으로 지원하는 버전은 1.13.0입니다. Python을 업그레이드하고 새 가상환경을 만든 뒤 2.0.0을 설치하세요. 즉시 전환하기 어렵다면 `typecast-autotag==1.13.0`으로 임시 고정할 수 있으나, EOL 런타임의 보안 지원이 복구되는 것은 아닙니다. 지원·테스트 대상은 기존 범위인 Python 3.10~3.13을 유지합니다. 이번 Python 전용 major 버전은 네이티브 라이브러리나 다른 언어 패키지를 변경하지 않습니다.
