...
const AUTO_TAG_PATTERNS = {
  ...
  address: {
    patterns: [
      // 괄호 안에 "~동," 형태가 있는 주소 패턴만 처리
      // 한글+숫자+동+숫자+호 + 괄호(~동,) (예: 우성베스토피아 102동 1101호 ( 엘지동, 우성베스토피아 ))
      /[가-힣a-zA-Z]+[가-힣a-zA-Z0-9\s]*\d{1,4}동\s*\d{2,4}호\s*\(\s*[가-힣]+동\s*,\s*[^)]+\)/g,
      // N동N호 + 괄호(~동,) (예: 113동2602호(엘지동, 더샵 염주센트럴파크))
      /\d{1,4}동\s*\d{2,4}호\s*\(\s*[가-힣]+동\s*,\s*[^)]+\)/g,
      // N-N호 + 괄호(~동,) (예: 210-405호(엘지동, 엘지주공아파트))
      /\d{1,4}-\d{1,4}호\s*\(\s*[가-힣]+동\s*,\s*[^)]+\)/g,
      // 괄호(~동,)로 시작하는 주소 + N동 N호 (예: (엘지동, 청명마을삼성아파트)439동 1006호)
      /\(\s*[가-힣]+동\s*,\s*[^)]+\)\s*\d{1,4}동\s*\d{2,4}호/g,
      // 숫자, + 괄호(~동,) + N동 N호 (예: 33, (엘지동, 청명마을삼성아파트)439동 1006호)
      /\d+,?\s*\(\s*[가-힣]+동\s*,\s*[^)]+\)\s*\d{1,4}동\s*\d{2,4}호/g,
      // N층 + N-N + N호 + 괄호(~동,) (예: 1층 111-2, 112호 (엘지동, 아파트))
      /\d{1,2}층\s+\d{1,4}-\d{1,4},?\s*\d{1,4}호\s*\(\s*[가-힣]+동\s*,\s*[^)]+\)/g,
      // N-N + 한글 + N층 (번지 + 가게명 + 층수) (예: 611-9 원조닭한마리 1층)
      /\d{1,5}-\d{1,5}\s+[가-힣]+[가-힣a-zA-Z0-9\s]*\d{1,2}층/g,
      // N동 N호 + 괄호(~동,) (공백 있는 경우) (예: 1011동 24호(엘지동, 엘지아이파크))
      /\d{1,4}동\s+\d{1,4}호\s*\(\s*[가-힣]+동\s*,\s*[^)]+\)/g,
