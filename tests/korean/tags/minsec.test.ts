...
describe('minsec', () => {
  ...
  describe('천단위 구분자 처리', () => {
    it('천단위 구분자가 있는 분을 처리한다 (1,234분)', () => {
      expect(minsec('1,234분')).toBe('천이백삼십사 분');
    });

    it('천단위 구분자가 있는 초를 처리한다 (1,234초)', () => {
      expect(minsec('1,234초')).toBe('천이백삼십사 초');
    });

    it('천단위 구분자가 있는 시간을 처리한다 (1,234시간)', () => {
      expect(minsec('1,234시간')).toBe('천이백삼십사 시간');
    });

    it('천단위 구분자가 있는 영문 분을 처리한다 (1,234m)', () => {
      expect(minsec('1,234m')).toBe('천이백삼십사 분');
    });

    it('천단위 구분자가 있는 영문 초를 처리한다 (1,234s)', () => {
      expect(minsec('1,234s')).toBe('천이백삼십사 초');
    });

    it('천단위 구분자가 있는 영문 시간을 처리한다 (1,234h)', () => {
      expect(minsec('1,234h')).toBe('천이백삼십사 시간');
    });

    it('천단위 구분자가 있는 복합 형식을 처리한다 (1,234m56s)', () => {
      expect(minsec('1,234m56s')).toBe('천이백삼십사 분 오십육 초');
    });
  });
});
