...
describe('autoTag - minsec (시분초 자동 태깅)', () => {
  describe('영문 형식', () => {
    it('NmNs 형식을 변환한다', () => {
      expect(autoMinsec('3m20s')).toBe('삼 분 이십 초');
      expect(autoMinsec('5m30s')).toBe('오 분 삼십 초');
    });

    it('Nm 형식을 변환한다', () => {
      expect(autoMinsec('5m')).toBe('오 분');
      expect(autoMinsec('30m')).toBe('삼십 분');
    });

    it('Ns 형식을 변환한다', () => {
      expect(autoMinsec('30s')).toBe('삼십 초');
      expect(autoMinsec('45s')).toBe('사십오 초');
    });
  ...
  describe('영문 풀 단어 형식', () => {
    it('minutes/seconds를 변환한다', () => {
      expect(autoMinsec('5 minutes')).toBe('오 분');
      expect(autoMinsec('30 seconds')).toBe('삼십 초');
    });
    ...
    it('min/sec 약어를 변환한다', () => {
      expect(autoMinsec('5min')).toBe('오 분');
      expect(autoMinsec('30sec')).toBe('삼십 초');
    });
  });

  describe('한글 형식', () => {
    it('N분N초를 변환한다', () => {
      expect(autoMinsec('3분20초')).toBe('삼 분 이십 초');
      expect(autoMinsec('5분 30초')).toBe('오 분 삼십 초');
    });

    it('N분만 변환한다', () => {
      expect(autoMinsec('10분')).toBe('십 분');
    });

    it('N초만 변환한다', () => {
      expect(autoMinsec('30초')).toBe('삼십 초');
    });
  ...
  describe('복수 시간', () => {
    it('여러 시간을 모두 변환한다', () => {
      ...
      expect(result).toContain('오 분');
      expect(result).toContain('삼십 초');
    });
  });
  ...
  describe('문맥 내 시간', () => {
    ...
    it('괄호 안 시간을 변환한다', () => {
      expect(autoMinsec('(3분 20초)')).toBe('(삼 분 이십 초)');
    });
  });
});
