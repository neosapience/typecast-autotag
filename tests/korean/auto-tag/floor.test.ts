...
describe('autoTag - floor (층수 자동 태깅)', () => {
  describe('일반 층수', () => {
    it('기본 층수를 변환한다', () => {
      expect(autoFloor('3층')).toBe('삼 층');
      expect(autoFloor('10층')).toBe('십 층');
    });

    it('문장 내 층수를 올바르게 변환한다', () => {
      expect(autoFloor('예약 장소: 강남 센터 3층')).toBe('예약 장소: 강남 센터 삼 층');
    });

    it('여러 층수를 모두 변환한다', () => {
      const result = autoFloor('1층 로비에서 만나 5층으로 이동');
      expect(result).toContain('일 층');
      expect(result).toContain('오 층');
    });
  });

  describe('지하층 - 영문 B', () => {
    it('B층 표기를 변환한다', () => {
      expect(autoFloor('B1층')).toBe('지하 일 층');
      expect(autoFloor('B2층')).toBe('지하 이 층');
    });
    ...
    it('문장 내 B층을 올바르게 변환한다', () => {
      expect(autoFloor('주차장은 B2층에 있습니다')).toBe('주차장은 지하 이 층 에 있습니다');
    });
  });

  describe('지하층 - 한글', () => {
    it('지하층 표기를 변환한다', () => {
      expect(autoFloor('지하1층')).toBe('지하 일 층');
      expect(autoFloor('지하2층')).toBe('지하 이 층');
    });

    it('지하와 숫자 사이에 공백이 있어도 처리한다', () => {
      expect(autoFloor('지하 1층')).toBe('지하 일 층');
    });

    it('문장 내 지하층을 올바르게 변환한다', () => {
      expect(autoFloor('창고는 지하3층에 있습니다')).toBe('창고는 지하 삼 층 에 있습니다');
    });
  });
  ...
  describe('복합 상황', () => {
    it('지상층과 지하층을 함께 변환한다', () => {
      ...
      expect(result).toContain('지하 일 층');
      expect(result).toContain('이십 층');
    });
  });
