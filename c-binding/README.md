...
- **다양한 패턴 지원**: 전화번호, 날짜, 시간, 금액, 순서 등 30+ 패턴 자동 인식
...
| `phone(번호)` | 전화번호 읽기 | `phone(010-1234-5678)` |
| `money(금액)` | 금액 읽기 | `money(50000)` → 오만 원 |
| `date(날짜)` | 날짜 읽기 | `date(2024-03-15)` |
| `time(시간)` | 시간 읽기 | `time(14:30)` |
| `year(연도)` | 연도 읽기 | `year(2024)` |
