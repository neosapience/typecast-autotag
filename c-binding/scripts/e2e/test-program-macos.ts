/**
 * macOS C Test Program Code
 *
 * This C code is compiled and executed locally on macOS.
 * Uses dlopen/dlsym to load the .dylib file.
 */

export const C_TEST_PROGRAM_MACOS = `
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <dlfcn.h>
...
typedef struct {
    const char* name;
    const char* input;
    const char* expected_contains; // String that must be in result (NULL to skip check)
    int should_change; // 1 if output must differ from input
} TestCase;
...
int main() {
    ...
    // Load library (macOS uses .dylib)
    void* handle = dlopen("libtypecast_autotag.dylib", RTLD_NOW);
    if (!handle) {
