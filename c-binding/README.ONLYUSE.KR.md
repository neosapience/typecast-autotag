...
> **참고:** 본 라이브러리에서 제공하는 **모든 수동 태그(38종)는 자동 태그 기능(`typecast_auto_tag`)에서도 지원**됩니다.
...
> **참고:** 아래 수동 태그들은 모두 자동 태그 기능(`typecast_auto_tag`)에서도 지원됩니다.
...
| `digits(숫자)`         | 한 자리씩   | `digits(123)` → 일 . 이 . 삼                    |
...
| `address(주소)`        | 주소        | `address(102동 1101호)` → 백이동 천백일호, `address(180-12번지)` → 백팔십 의 십이 번지 |
...
> **참고:** 모든 수동 태그는 자동 태그 기능(`typecast_auto_tag`)에서도 동일하게 지원됩니다.

### v1.5.0 ~ v1.7.0

- 영어 자동 태깅 기능 추가
- 주소(address) 태깅 기능 강화
  - 번지 패턴 지원 추가 (예: `180-12번지` → 백팔십 의 십이 번지)
  - 도로명 주소 패턴 지원 추가
  - 아파트 동/호수 패턴 지원 (예: `102동 1101호` → 백이동 천백일호)
- 한국어 출력 형식 통일 (구분자 점(.) 표기 일관성 개선)
- 다양한 주소 형식 자동 인식 및 변환 지원

### v1.4.0

- 영어 지원 추가
- 데모 프로젝트 추가 (React + Vite)
- 문서화 개선 (한국어/영어)
